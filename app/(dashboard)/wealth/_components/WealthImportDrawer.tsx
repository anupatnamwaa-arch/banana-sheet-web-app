...
const FIELD_LABELS: { field: keyof WealthColumnMapping; label: string; required: boolean }[] = [
  ...
  { field: "type", label: "ประเภท (asset/liability)", required: true },
  { field: "value", label: "มูลค่า (฿)", required: true },
  { field: "is_liquid", label: "สภาพคล่อง (true/false)", required: false },
  { field: "monthly_payment", label: "ยอดผ่อน/เดือน", required: false },
  { field: "due_date", label: "ครบกำหนด (YYYY-MM-DD)", required: false },
];
...
function mapWealthRow(
  ...
  if (!name) return { row: null, error: "ไม่มีชื่อ" };
  ...
  const type =
    rawType === "asset" || rawType === "สินทรัพย์"
      ...
      ? "liability"
      : null;
  if (!type) return { row: null, error: `ประเภทไม่ถูกต้อง: "${rawType}" (ต้องเป็น asset หรือ liability)` };

  const rawValue = raw[mapping.value]?.trim() ?? "";
  const value = parseFloat(rawValue.replace(/[^0-9.]/g, ""));
  if (!Number.isFinite(value) || value < 0) return { row: null, error: `มูลค่าไม่ถูกต้อง: "${rawValue}"` };
...
export function WealthImportDrawer({ onSuccess }: Props) {
  ...
  const previewRows = parsed
    ? parsed.rows.slice(0, 5).map((r) => mapWealthRow(r, mapping))
    : [];
  const canConfirm = !!mapping.name && !!mapping.type && !!mapping.value && parsed !== null;
  const validCount = parsed
    ? parsed.rows.filter((r) => mapWealthRow(r, mapping).row !== null).length
    : 0;
  ...
  async function handleConfirm() {
    ...
    try {
      const validRows = parsed.rows.flatMap((r) => {
        const { row } = mapWealthRow(r, mapping);
        return row ? [row] : [];
      });
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold">นำเข้าสินทรัพย์/หนี้สิน</h2>
              ...
              <div className="rounded-xl bg-[var(--glass-bg)] p-3 text-xs text-fg-muted">
                <p className="mb-1 font-medium text-fg">รูปแบบไฟล์ที่รองรับ</p>
                <p>คอลัมน์ที่จำเป็น: <span className="text-accent">name, type, value</span></p>
                <p>คอลัมน์เสริม: is_liquid, monthly_payment, due_date</p>
                <p className="mt-1">ค่า type: <code>asset</code> หรือ <code>liability</code></p>
              </div>

              {result ? (
                <div className="flex flex-col items-center gap-3 py-8 text-center">
                  <CheckCircle2 size={40} className="text-positive" />
                  <p className="text-lg font-semibold">✓ นำเข้าสำเร็จ {result.inserted} รายการ</p>
                  {result.skipped > 0 && (
                    <p className="text-sm text-fg-muted">ข้ามไป {result.skipped} รายการ</p>
                  )}
                  <button
                ...
                <>
                  ...
                  >
                    ...
                    <p className="text-sm font-medium">
                      {isDragActive ? "วางไฟล์ที่นี่" : "วาง CSV / XLSX หรือแตะเพื่อเลือก"}
                    </p>
                  </div>
                  ...
                  {parsed && (
                    <div className="space-y-4">
                      {/* Column mapping */}
                      <p className="text-sm font-medium">จับคู่คอลัมน์</p>
                                  ...
                                  r.row ? (
                                    <tr key={i} className="border-b border-[var(--glass-border)]/50">
                                      ...
                                      <td className={`px-3 py-2 ${r.row.type === "asset" ? "text-positive" : "text-negative"}`}>
                                        {r.row.type === "asset" ? "สินทรัพย์" : "หนี้สิน"}
                                      </td>
