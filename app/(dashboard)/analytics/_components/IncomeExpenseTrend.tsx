...
function ChartTooltip({ active, payload, label: l, t }: any) {
  if (!active || !payload?.length) return null;
  return (
    <div className="glass rounded-xl px-3 py-2 text-xs shadow-lg">
      <p className="mb-1 font-medium">{l}</p>
      <p className="text-positive">{t.analytics.legendIncome} {formatTHB(payload[0]?.payload.income ?? 0)}</p>
      <p className="text-negative">{t.analytics.legendExpense} {formatTHB(payload[0]?.payload.expense ?? 0)}</p>
      <p className="text-blue-400">{t.analytics.legendSavings} {formatTHB(payload[0]?.payload.savings ?? 0)}</p>
    </div>
  );
}
...
export function IncomeExpenseTrend({ trend, currentMonthRemaining }: Props) {
  ...
  function label(month: string): string {
    ...
    return `${months[idx] ?? month} ${y + yearOffset}`;
  }
  ...
  return (
    <div className="rounded-[var(--radius-card)] bg-[var(--bg-elevated)] p-4">
      <p className="mb-1 text-sm font-semibold">{t.analytics.incomeTrendTitle}</p>
      <div className="mb-3 flex gap-3 text-[11px] text-fg-muted">
        <span className="flex items-center gap-1">
          <span className="h-2 w-2 rounded-sm bg-positive" />{t.analytics.legendIncome}
        </span>
        <span className="flex items-center gap-1">
          <span className="h-2 w-2 rounded-sm bg-negative" />{t.analytics.legendExpense}
        </span>
        <span className="flex items-center gap-1">
          <span className="h-2 w-2 rounded-sm bg-blue-400" />{t.analytics.legendSavings}
        </span>
      </div>
      ...
      <p className="mt-3 text-xs text-fg-muted">
        {t.analytics.monthRemainingTemplate}{" "}
