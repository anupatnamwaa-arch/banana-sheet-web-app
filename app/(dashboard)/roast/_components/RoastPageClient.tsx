// app/(dashboard)/roast/_components/RoastPageClient.tsx
"use client";
...
interface Props {
  initialRoasts: PastRoast[];
}

export function RoastPageClient({ initialRoasts }: Props) {
  ...
  const [roasts, setRoasts] = useState<PastRoast[]>(initialRoasts);
  ...
  return (
    <section className="space-y-4 pb-32">
      ...
      {roasts.length > 0 && !streaming && (
        <RoastHistory roasts={roasts} />
      )}
    </section>
  );
}
